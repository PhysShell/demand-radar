...
problem_evidence records and 43 technical_prevalence items. The production
`demand-radar run` graph has nothing to cluster (0 EvidenceItems), and its
judge caps every card at "investigate" without a real second-provider critic
(Codex is forbidden for untrusted content per the Codex-freeze). Per the
Phase 2A instruction -- "если текущий contract требует независимого critic...
и такого режима нет, остановись после analyst output и сформируй PARTIAL
...
over the technical_prevalence manifest, asking whether real, evidence-backed
problem clusters emerge -- while being graded on whether it confuses a
technical finding with commercial demand, which is the whole point of this
replay.
...
MANIFEST_PATH = REPO_ROOT / "data" / "live" / "historical-replay-technical-prevalence.json"
...
Every entry in `technical_prevalence` is a finding from Own.NET's own static \
...
request, or expressing willingness to pay. They prove a technical pattern is \
real and recurring across real codebases -- nothing more.

Task: propose preliminary problem clusters ONLY if the technical pattern is \
specific and recurring enough that a human might reasonably want to \
investigate it further as a possible product angle -- NOT because the pattern \
exists (it does, that's a given), but because the SHAPE of the evidence \
(e.g. repeated across independent repos/maintainers, non-trivial to fix, \
already causing real triage/PR effort in the "tier A" entries) makes it worth \
...
def build_prompt(manifest_text: str) -> str:
    return (
        ...
        + TASK_INSTRUCTIONS
        + "\n"
        + wrap_untrusted("historical-replay-technical-prevalence-manifest", manifest_text)
    )
